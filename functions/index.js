...
exports.hubungiKamiChatNotification = functions.database.ref("hubungiKamiMessage/{senderUid}/{adminUid}/{hubungiKamiMessageUid}")
	.onWrite(async (change, context) => {
		...
		const results = await Promise.all([getDeviceTokensPromise, getAdminPromise]);
		...
		const usernameSnapShopt = follower.val();
		const username = usernameSnapShopt.username;
		console.log('Fetched follower profile >>>>>>> ', username);


		//const result = await getAdminPromise;
		//Get username of the admin
		//const username = follower.val().username;
